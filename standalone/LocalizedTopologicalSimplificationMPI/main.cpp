...
#include <ttkLocalizedTopologicalSimplificationMPI.h>
...
int main(int argc, char **argv) {
  ...
  // ---------------------------------------------------------------------------
  ttk::Debug msg;
  msg.setDebugMsgPrefix("LocalizedTopologicalSimplificationMPI");

  // ---------------------------------------------------------------------------
  // Initialize ttkLocalizedTopologicalSimplificationMPI module (adjust parameters)
  // ---------------------------------------------------------------------------
  auto localizedTopologicalSimplificationMPI = vtkSmartPointer<ttkLocalizedTopologicalSimplificationMPI>::New();

  // ---------------------------------------------------------------------------
  // TODO 14: Pass custom arguments and options to the module
  // ---------------------------------------------------------------------------
  // localizedTopologicalSimplificationMPI->SetOutputArrayName(outputArrayName);
  ...
  for(size_t i = 0; i < inputFilePaths.size(); i++) {
    ...
    if(listArrays) {
      ...
      // feed input object to ttkLocalizedTopologicalSimplificationMPI filter
      localizedTopologicalSimplificationMPI->SetInputDataObject(i, reader->GetOutput());
  ...
  for(size_t i = 0; i < inputArrayNames.size(); i++)
    localizedTopologicalSimplificationMPI->SetInputArrayToProcess(i, 0, 0, 0, inputArrayNames[i].data());

  // ---------------------------------------------------------------------------
  // Execute ttkLocalizedTopologicalSimplificationMPI filter
  // ---------------------------------------------------------------------------
  localizedTopologicalSimplificationMPI->Update();

  // ---------------------------------------------------------------------------
  // If output prefix is specified then write all output objects to disk
  // ---------------------------------------------------------------------------
  if(!outputPathPrefix.empty()) {
    for(int i = 0; i < localizedTopologicalSimplificationMPI->GetNumberOfOutputPorts(); i++) {
      auto output = localizedTopologicalSimplificationMPI->GetOutputDataObject(i);
